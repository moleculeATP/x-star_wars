...
uniform int active_lights[20];
uniform float d_light_max[20];
...
void main()
{
   // Compute the position of the center of the camera
    ...
    for (int i = 0; i < N_lights; i++) {
        ...
        float d_lmax = d_light_max[i];
        if (d_lmax > 0) {
            float d_light = length(light_positions[i] - fragment.position);
        ...
        */
        ...
        vec3 L = normalize(light_positions[i]-fragment.position);
        ...
        if(diffuse_component > 0.0){
            ...
            specular_component = pow( max(dot(R,V),0.0), coef_exp );
        }
        ...
        vec3 color_object = fragment.color * material.color * color_image_texture.rgb * light_colors[i];

        // Compute the final shaded color using Phong model
        float Ka = ambiant;
        float Kd = diffus;
        float Ks = coef_spec;
        vec3 color_shading = (Ka + Kd * diffuse_component) * color_object + Ks * specular_component * light_colors[i];
